...
class osvoice{
    ...
    public function getSettings(){
        $result = select_query("mod_osvoice_settings", "*");
    ...
    function send($len,$n){
        ...
        if($sender_function == false){
            ...
            include_once("callers/farapayamak.php");
    ...
    function getBalance(){
        ...
        if($sender_function == false){
            ...
            include_once("callers/".$sender_function.".php");
    ...
    function getReport($msgid){
        $result = mysql_query("SELECT sender FROM mod_osvoice_messages WHERE msgid = '$msgid' LIMIT 1");
    ...
    function getSenders(){
        if ($handle = opendir(dirname(__FILE__).'/callers')) {
            while (false !== ($entry = readdir($handle))) {
                if(substr($entry,strlen($entry)-4,strlen($entry)) == ".php"){
                    $file[] = require_once('callers/'.$entry);
                }
            }
            closedir($handle);
        }
    ...
    function checkHooks($hooks = null){
        ...
        foreach($hooks as $hook){
            $sql = "SELECT `id` FROM `mod_osvoice_templates` WHERE `name` = '".$hook['function']."' AND `type` = '".$hook['type']."' LIMIT 1";
            ...
            if($num_rows == 0){
                if($hook['type']){
                    ...
                    insert_query("mod_osvoice_templates", $values);
                    $i++;
                }
            }else{
                $values = array(
                ...
                update_query("mod_osvoice_templates", $values, "name = '" . $hook['name']."'");
            }
        }
        return $i;
    }
